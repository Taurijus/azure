...
function PlayerI.load()
	...
	PlayerI.image = love.graphics.newImage("images/4.png")
...
if Shop.OnShop(PlayerI.x, PlayerI.y) >0 and Inventory.loaded() == 0 then Inventory.AddWeapon(Shop.OnShop(PlayerI.x, PlayerI.y)) end
local dx, dy
dx = x-PlayerI.x
dy = y-PlayerI.y

PlayerI.rot = math.acos(dx/(math.sqrt(dx*dx+dy*dy)))*(dy/math.abs(dy))
if PlayerI.rot == nil then PlayerI.rot = 0 end
...
function PlayerI.draw()
	...
	love.graphics.translate(-PlayerI.x, -PlayerI.y)
	love.graphics.draw(PlayerI.image, PlayerI.x-32, PlayerI.y-32)	
end
