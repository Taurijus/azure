...
function gamemap.load()
	...
	gamemap.image1 = love.graphics.newImage("images/6.png")
	gamemap.image2 = love.graphics.newImage("images/6.png")
	gamemap.x = 0
	gamemap.y = 0
	gamemap.xoffset = 0
	gamemap.yoffset = 0
...
function gamemap.voidTouch()
	...
	for i = 1, #gamemap.void do
		...
		local x1 = gamemap.void[i].x1+gamemap.x
		local y1 = gamemap.void[i].y1+gamemap.y
		local x2 = gamemap.void[i].x2+gamemap.x
		local y2 = gamemap.void[i].y2+gamemap.y
...
function gamemap.wallTouch()
	...
	for i = 1, #gamemap.wall do
                for j = 1, #gamemap.wall[i] do
                       local width = love.graphics.getWidth()
			local height = love.graphics.getHeight()
			local x1 = gamemap.wall[i][j].x1+gamemap.x
			local y1 = gamemap.wall[i][j].y1+gamemap.y
			local x2 = gamemap.wall[i][j].x2+gamemap.x
			local y2 = gamemap.wall[i][j].y2+gamemap.y
...
function gamemap.draw(xxx,yyy)
	love.graphics.draw(gamemap.image, gamemap.x-950, gamemap.y-190)
	love.graphics.translate( gamemap.x, gamemap.y)
	love.graphics.scale(1.2*1,1.2*0.65)
	...
	love.graphics.draw(gamemap.image1, xxx, yyy, 0, 1, 1, 5, 5)
	love.graphics.rotate( -math.pi/4 )
	love.graphics.scale(1/(1.2*1),1/(0.65*1.2))
	love.graphics.translate( -gamemap.x, -gamemap.y  )
	love.graphics.draw(gamemap.image2, xxx, yyy, 0, 1, 1, 5, 5)
...
--	love.graphics.translate( gamemap.x, gamemap.y  )
		...
		for i = 1, #gamemap.wall do
                for j = 1, #gamemap.wall[i] do
                        --love.graphics.print(tostring(gamemap.wall[i][j].x1), j * 50, i * 10)
			local xx1 = gamemap.wall[i][j].x1+gamemap.x+700
			local yy1 = gamemap.wall[i][j].y1+gamemap.y
			local xx2 = gamemap.wall[i][j].x2+gamemap.x+700
			local yy2 = gamemap.wall[i][j].y2+gamemap.y
			love.graphics.line(xx1,yy1,xx2,yy2)
			love.graphics.rectangle("line", xx1, yy1, xx2-xx1, yy2-yy1)
                end
        end
	for i = 1, #gamemap.void do
		local x1 = gamemap.void[i].x1+gamemap.x+700
		local y1 = gamemap.void[i].y1+gamemap.y
		local x2 = gamemap.void[i].x2+gamemap.x+700
		local y2 = gamemap.void[i].y2+gamemap.y
		love.graphics.line(x1,y1,x2,y2)
		love.graphics.rectangle("line", x1, y1, x2-x1, y2-y1)
	end
